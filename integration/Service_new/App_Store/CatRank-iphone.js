    ...
    for (let i = 0; i <= favcountryID.length - 1; i++) {
        ...
        context('Check response from Category Ranking iPhone. Local ' + favcountryID[i], () => {
            ...
            it('Category Ranking API. Check data on y != null', () => {
                ...
                if (favcountryID[i] === 'CN') {
                    cy.request({
                        method: 'get',
                        followRedirect: false, log: true, //turn off
                        url: 'api/category-ranking/chart?category=' + appCategoryID.get('aps-cn') + '&category_list=free&country=' + favcountryID[i] + '&device_type=' + deviceType.get('iphone') + '&storeids=' + storeID.get('aps-cn') + '&timestamp_since=' + prevdaysDate,
                        headers: {
                    ...
                    cy.request({
                        method: 'get',
                        followRedirect: false, log: true, //turn off
                        url: 'api/category-ranking/chart?category=' + appCategoryID.get('aps') + '&category_list=free&country=' + favcountryID[i] + '&device_type=' + deviceType.get('iphone') + '&storeids=' + storeID.get('aps') + '&timestamp_since=' + prevdaysDate,
                        headers: {
                            'accept': 'application/json'
                        },
                        response: []
