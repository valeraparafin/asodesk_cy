...
import {Features, Tariffs, User_Permissions} from './Classes_library/tariffs'
...
for (let account in accounts.Plans) {
    describe(account + ' tariff should be exactly same as in the document  ', () => {
        ...
        if (tariffs.Plans) {
            ...
            it('Compare ' + account + ' permitions with server JSON', function () {

                const Model_Limits2 = userPermissions.withTariff(account)
                ...
                let User_Permissions = {
                    'Basic': {
                        ...
                        App_Overview:feature.user.available_features["custom-reports-aso-report"].no_show,
                        ASO_Report:feature.user.available_features["custom-reports-app-overview-report"].no_show,
                    ...
                    'Startup_ASO_300': {
                        ...
                        App_Overview:feature.user.available_features["custom-reports-aso-report"].no_show,
                        ASO_Report:feature.user.available_features["custom-reports-app-overview-report"].no_show,
                        Keyword_Report:feature.user.available_features["custom-reports-keywords-report"].no_show,
                        Review_Report:feature.user.available_features["custom-reports-reviews-report"].no_show,
                        Alerts_Updates:feature.user.available_features["custom-reports-app-updates-alert"].no_show,
...
function comparePermissions(Local, Prod) {
    if (acc === 'Basic') {
        chai.expect(Local.limited_aso_cr).eq(Prod.limited_aso_cr, 'Comparative Report Graph');
    ...
    else if (acc === 'Startup_ASO_300') { console.log('Startup PUIZDEC')}
